...
import type { ActividadV2, HabitoV2, TareaV2 } from "../registros/ActividadV2";
import {
  convertirPoliticaEnV1,
  rehidratarPoliticaDesdeV1,
} from "./MapeadorPoliticaCompromisoV1";

export type CodigoErrorMapeoActividadV2 =
  | "VERSION_ACTIVIDAD_NO_SOPORTADA"
  | "REGISTRO_ACTIVIDAD_INVALIDO"
  | "MODO_SEGUIMIENTO_INVALIDO";

export class ErrorMapeoActividadV2 extends Error {
  constructor(
    public readonly codigo: CodigoErrorMapeoActividadV2,
    mensaje: string,
    public readonly causa?: unknown,
  ) {
    super(mensaje);
    this.name = "ErrorMapeoActividadV2";
  }
}

export function convertirActividadEnV2(actividad: Actividad): ActividadV2 {
  const politicaPredeterminada = actividad.obtenerPoliticaPredeterminada();
  const base = {
    versionEsquema: 2 as const,
    id: actividad.id,
  ...
  throw new ErrorMapeoActividadV2(
    "REGISTRO_ACTIVIDAD_INVALIDO",
    "El tipo concreto de actividad no puede persistirse como ActividadV2.",
  );
}

export function migrarActividadV1AV2(registro: ActividadV1): ActividadV2 {
  if (Number(registro.versionEsquema) !== 1) {
    throw new ErrorMapeoActividadV2(
      "VERSION_ACTIVIDAD_NO_SOPORTADA",
      `La versión ${String(registro.versionEsquema)} no puede migrarse a ActividadV2.`,
    );
  }
  return Object.freeze({
    ...registro,
...
export function rehidratarActividadDesdeV2(registro: ActividadV2): Actividad {
  if (Number(registro.versionEsquema) !== 2) {
    throw new ErrorMapeoActividadV2(
      "VERSION_ACTIVIDAD_NO_SOPORTADA",
      `La versión ${String(registro.versionEsquema)} de Actividad no está soportada.`,
    );
  }
  if (!esModoSeguimiento(registro.modoSeguimiento)) {
    throw new ErrorMapeoActividadV2(
      "MODO_SEGUIMIENTO_INVALIDO",
      `El modo de seguimiento ${String(registro.modoSeguimiento)} no está soportado.`,
    );
  }
  ...
  try {
    ...
    if (error instanceof ErrorMapeoActividadV2) throw error;
    throw new ErrorMapeoActividadV2(
      "REGISTRO_ACTIVIDAD_INVALIDO",
      `El registro ActividadV2 no satisface las invariantes del dominio.${error instanceof Error ? ` ${error.message}` : ""}`,
      error,
    );
  }
}

function rehidratarTarea(registro: TareaV2): Tarea {
  return Tarea.rehidratar({
    id: registro.id,
    titulo: registro.titulo,
    tipo: registro.tipo,
    modoSeguimiento: registro.modoSeguimiento,
...
function rehidratarHabito(registro: HabitoV2): Habito {
  return new Habito({
    id: registro.id,
    titulo: registro.titulo,
    modoSeguimiento: registro.modoSeguimiento,
